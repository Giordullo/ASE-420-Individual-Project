...
class Console:
    ...
    def report(param):
        db = Database()
        from_time = dateparser.parse(param[1])
        to_time = dateparser.parse(param[2])
        data = db.getTasks()
        ...
        data = filter(lambda task: from_time.date() <=
                      task.date.date() <= to_time.date(), data)
    ...
    def priority(param):
        db = Database()
        data = db.getTasks()
        for key, group in itertools.groupby(data, lambda task: task.tag):
            millis_spent = 0
            for task in group:
                millis_spent += (task.to_time -
                                 task.from_time).total_seconds() * 1000
            print("{}: {}".format(key, timedelta(milliseconds=millis_spent)))

    @staticmethod
    def runConsole():
        print("Input in a Command (record / query / report / priority)")
        while (True):
            try:
                data = input("> ")
                ...
                if (command == 'record'):
                    ...
                    Console.report(data_split)
                elif (command == 'priority'):
                    Console.priority(data_split)
                else:
